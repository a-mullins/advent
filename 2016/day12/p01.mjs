...
// This means that the p02 symbolic link trick won't work:
// if (process.argv[1] === fileURLToPath(import.meta.url)) {
if(process.argv.includes("-D")) {var DEBUG = true;}

let text = opt(fs.readFileSync(0, "ascii"))
    .split("\n")
    .filter(line => line.length > 0)
    .map(line => line.split(" "));

let r = run(text, process.argv[1].includes("p02") ? {"c": 1} : undefined);
