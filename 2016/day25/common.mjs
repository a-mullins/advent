...
function* run(text, registers, DEBUG) {
    const r = {...{"a": 0, "b": 0, "c": 0, "d": 0}, ...registers};
    let ip = 0;
    //const output = [];

    function _cur_state() {
        return {text: text, ip: ip, r: {...r}, output: Array.from(output)};
    }
...
// Array.from({length: 2}, function(){return this.next().value;}, run(text));
// Array.from({length: 5}, function(){return this.next().value;}, gen());
